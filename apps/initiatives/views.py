...
from .models import PROBLEM_QUESTIONS, Initiative, Problem, Solution
...
def youth_view(request, problem_id=None):
    """/tashabbuslar/yoshlar — muammolar ro'yxati va yechim taklif qilish."""
    problems = (Problem.objects.filter(is_published=True)
                .select_related('organization')
                .annotate(solution_count=Count('solutions'))
                .order_by('-created_at'))

    selected = get_object_or_404(Problem, pk=problem_id) if problem_id else None

    if request.method == 'POST':
        selected = get_object_or_404(Problem, pk=request.POST.get('problem_id'))
        form = SolutionForm(request.POST, request.FILES, user=request.user)
        if form.is_valid():
            solution = form.save(commit=False)
            solution.problem = selected
            if request.user.is_authenticated:
                solution.author = request.user
            solution.save()
            messages.success(request, "Yechimingiz muvaffaqiyatli yuborildi!")
            return redirect('initiatives:solution_success')
    else:
        form = SolutionForm(user=request.user)

    return render(request, 'initiatives/youth.html', {
        'problems': problems,
        'selected': selected,
        'form': form,
    })
